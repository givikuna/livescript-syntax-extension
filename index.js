...
(function () {
    ...
    function countOccurrences(key, toSearch) {
        ...
        if (
            (typeof toSearch === "string" && typeof key === "string") ||
            typeof key === "number" ||
            typeof key === "boolean"
        ) {
            key = str(key);
    ...
    function stringToNumber(it) {
        ...
        if (
            typeof it !== "string" ||
            /^\d+[a-zA-Z]*$/.test(it === false) ||
            (countOccurrences("-", it > 0) && !it.startsWith("-"))
        ) {
            return 0;
        }
        it = supertrim(it);
        style = (function (inputString) {
            if (
                countOccurrences(".", inputString) > 1 &&
                countOccurrences(",", inputString) < 2 &&
                countOccurrences("_", inputString) === 0
            ) {
                return "european";
            }
            if (
        ...
        try {
            return Number(
                (function (givenStyle, givenString) {
                    var returnString;
                    returnString = "";
    ...
    function input(prompt, changeTo) {
        ...
        if (
            (changeTo === "num" || changeTo === "n" || changeTo === "number" || changeTo === "int") &&
            isNumeric(answer)
        ) {
            answer = int(answer);
        }
    ...
    function getFileExtension(fileName) {
        ...
        if (isMoreThan(len(fileName.split(".")), 1)) {
            return str(file.split(".")).pop();
        }
        return str(undefined);
    }
    function execute(command) {
        return str(execSync(command));
    }
    module.exports = {
        execute: runCommand,
